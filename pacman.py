...
def bfs_pathfinding(start, end, walls):
    ...
    while queue:
        ...
        for dx, dy in directions:
            ...
            if next_pos not in visited:
                ...
                dummy_wall = Wall(next_pos[0], next_pos[1], 30, 30, blue)
                if not pygame.sprite.spritecollide(dummy_wall, walls, False):
...
class Ghost(Player):
    ...
    def changespeed(self, list=None, ghost=None, turn=0, steps=0, l=0, dx=None, dy=None):
        if dx is not None and dy is not None:
            # Pathfinding mode: directly set speed
            self.change_x = dx
            self.change_y = dy
    ...
    while done == False:
        ...
        while not done:
            ...
            for ghost in [Pinky, Blinky,]:
                ...
                if path:  # If a path is found, move the ghost
                    dx, dy = path[0]  # Get the next step in the path
                    ghost.changespeed(dx=dx, dy=dy)  # Use pathfinding mode
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    del wall_list
                    del gate
                    level = show_level_select()  # Show level selection again
                    startGame(level)  # Restart the game with the chosen level
